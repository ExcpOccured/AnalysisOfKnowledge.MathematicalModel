...
double CalculateDeterminate(Matrix_t *matrix, int executedDimension) {
    ...
    for (int index = 0; index < executedDimension; index++) {
        ...
        determinate +=
                sign * matrix->MatrixHeader[0][index] * CalculateDeterminate(destinationMatrix, executedDimension - 1);
...
void AdjointMatrix(Matrix_t *rank, Matrix_t *sourceMatrix) {
    ...
    double buffer[executableDimension][executableDimension];

    Matrix_t destinationMatrix = {
            .Dimension = executableDimension,
            .MatrixHeader = (double **)buffer
    };
    ...
    for (int iIndex = 0; iIndex < executableDimension; iIndex++) {
        for (int jIndex = 0; jIndex < executableDimension; jIndex++) {
            ...
            sourceMatrix->MatrixHeader[jIndex][iIndex] =
                    (sign) * (CalculateDeterminate(&destinationMatrix, executableDimension - 1));
        }
    }
}
    ...
    for (int iIndex = 0; iIndex < executableDimension; iIndex++) {
        for (int jIndex = 0; jIndex < executableDimension; jIndex++) {
            ...
            for (int kIndex = 0; kIndex < executableDimension; kIndex++) {
                productionMatrix->MatrixHeader[iIndex][jIndex] +=
                        firstMatrix->MatrixHeader[iIndex][kIndex] * secondMatrix->MatrixHeader[kIndex][jIndex];
            }
        }
    }
